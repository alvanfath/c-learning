            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.guru.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group row mb-2">
                            <label for="password" class="col-form-label col-sm-12 col-md-12"><small class="text-danger">*</small> password</label>
                            <div class="col-sm-12 col-md-12">
                            ...
                            <div class="col-sm-12 col-md-12">
                                <select name="tingkat_id" class="form-control select2" id="tingkat">
                                    ...
                                    @foreach ($tingkat as $item)
                                        <option value="{{ $item->id }}" @if(old('tingkat_id') == $item->id) selected @endif>{{ $item->tingkat_kelas }}</option>
                                    @endforeach
                                </select>
                                @error('tingkat_id')
                                    <small class="text-danger">{{ $message }}</small>
                        ...
                        <div class="form-group row mb-2">
                            ...
                            <div class="col-sm-12 col-md-12">
                                <select name="mapel_id" class="form-control select2" id="mapel_form">
                                    @if (Session::has('mapel'))
                                        @php
                                            $mapel = Session::get('mapel')
                                        @endphp
                                        <option value="" selected disabled>--Pilih Mapel--</option>
                                        @foreach ($mapel as $item)
                                        <option value="{{ $item->id }}" @if(old('mapel_id') == $item->id) selected @endif>{{ $item->nama }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('mapel_id')
